...
def generate_recommendation(wine):
    ...
    # Минимальная розничная цена с наценкой 30% (ниже продавать в убыток)
    min_retail_price = pur_price * 1.3
    
    # Конкурент активен, если он в наличии и его цена со скидкой (итоговая) больше нуля
    ...
    # Ищем конкурента с самой низкой итоговой ценой (disc)
    cheapest = min(active_comps, key=lambda x: x['disc'])
    c_price = cheapest['disc'] # Это чистая цена конкурента на полке (например, 2000₽)
    
    target = c_price * 1.1 # Целевая цена с учетом 10% наценки за наш премиальный сервис
    
    if target < min_retail_price:
        return f"🛑 **Капитуляция с честью:** У конкурента ({cheapest['shop']}) минимальная цена {c_price:.0f}₽. Наша целевая цена с учетом сервиса ({target:.0f}₽) падает ниже минимально допустимой розницы ({min_retail_price:.0f}₽). Не снижайте цену, делайте упор на сервис и уникальность."
        
    # Если у конкурента цена со скидкой равна регулярной (то есть скидки нет)
    ...
    for d in discounts:
        ...
        # Ищем минимальную скидку, которая удерживает нас выше минимальной розницы, но бьет цель
    ...
    if best_d is not None:
         if best_d[0] == 0:
             ...
             return f"⚔️ **Борьба скидками:** Рекомендуемая скидка **{best_d[0]}%** (Ваша цена: {best_d[1]:.0f}₽). Целевая цена конкуренции: {target:.0f}₽."
             
    # Если фиксированные скидки не подходят (режут минимальную розницу) -> Пакетные акции
    promo = []
    ...
    if promo:
         return "🎁 **Пакетные акции:** Обычные поштучные скидки слишком сильно режут маржу. Чтобы конкурировать, используйте объём:\n" + "\n".join([f"- {x}" for x in promo])
         
    return f"⚖️ Обычные скидки не достигают целевой цены {target:.0f}₽ без нарушения минимальной маржи. Отредактируйте 'Нашу Рег. цену' вверх или продавайте без скидок, делая ставку на сервис."
