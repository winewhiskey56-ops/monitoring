            ...
            if not all_docs:
                ...
                if not filtered_texts:
                    ...
                    with st.spinner(f"ИИ анализирует накладные со снайперской точностью ({len(filtered_texts)} шт.)..."):
                        ...
                        # --- ОБНОВЛЕННЫЙ И ЖЕСТКИЙ ПРОМПТ ДЛЯ КОРРЕКТНОГО АНАЛИЗА ЦЕН ---
                        prompt = (
                            "Ты — эксперт-аналитик первичных документов и менеджер базы данных винного магазина. "
                            "Твоя задача — найти точную реальную закупочную цену товара в предоставленном тексте накладных.\n\n"
                            f"Искомый товар: {product_search}\n\n"
                            f"ТЕКСТЫ НАКЛАДНЫХ:\n{full_invoices_text}\n\n"
                            "⚠️ КРИТИЧЕСКИ ВАЖНЫЕ ПРАВИЛА ДЛЯ ОПРЕДЕЛЕНИЯ ЦЕНЫ (ключ 'price'):\n"
                            "1. ВСЕГДА ищи и бери значение из колонки 'Цена со скидкой', 'Цена с учетом скидки', 'Цена со ск.' или 'Итоговая цена за единицу'. Это твой главный приоритет!\n"
                            "2. Если отдельной колонки со скидкой нет, но указана базовая цена и процент скидки в строке — самостоятельно высчитай реальную финальную цену закупа.\n"
                            "3. Категорически ЗАПРЕЩЕНО брать розничные цены, МРЦ (минимальные розничные цены) или рекомендуемые цены для продажи, если они указаны в документе. Нам нужна только наша чистая закупочная стоимость.\n"
                            "4. Записывай в 'price' только число (цифры, можно с точкой для копеек), без букв 'руб' или символов '₽'.\n\n"
                            "ДОПОЛНИТЕЛЬНЫЕ ИНСТРУКЦИИ:\n"
                            "- ОБЯЗАТЕЛЬНО найди в тексте каждого документа дату его составления (обычно вверху накладной рядом с номером договора или ТОРГ-12).\n"
                            "- Если товар найден в нескольких файлах, выведи все упоминания (историю цен).\n\n"
                            "Ответь строго в формате JSON-массива объектов с ключами: product, found_name, price, date, invoice, status.\n"
                            "Где:\n"
                            "  'product' - твой исходный запрос;\n"
                            "  'found_name' - точное наименование позиции, как оно написано внутри самой накладной;\n"
                            "  'price' - финальная чистая закупочная цена (желательно со скидкой);\n"
                            "  'date' - найденная дата документа;\n"
                            "  'invoice' - имя файла (название накладной);\n"
                            "  'status' - короткий комментарий (например: 'Цена со скидкой', 'Прямая цена без скидки')."
                        )
