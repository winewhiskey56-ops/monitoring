...
def generate_recommendation(wine):
    ...
    min_margin = pur_price * 1.3
    active_comps = [c for c in wine['competitors'] if c['in_stock'] and (c['reg'] - c['disc']) > 0]
    
    if not active_comps:
         return "✅ **Вне конкуренции:** Конкурентов нет или товар у них отсутствует. Рекомендуется убрать скидку (0%)."
         
    cheapest = min(active_comps, key=lambda x: x['reg'] - x['disc'])
    c_price = cheapest['reg'] - cheapest['disc']
    target = c_price * 1.1 # Целевая цена с учетом 10% сервиса
    
    if target < min_margin:
        return f"🛑 **Капитуляция с честью:** У конкурента ({cheapest['shop']}) цена {c_price}₽. Наша целевая цена ({target:.0f}₽) падает ниже минимальной маржи ({min_margin:.0f}₽). Не снижайте цену, делайте упор на сервис."
        
    if cheapest['disc'] == 0:
        return f"🎯 **Фиксированная цена:** Конкурент ({cheapest['shop']}) торгует без скидок. Поставьте скидку 0% и установите верхнюю цену около {target:.0f}₽."
        
    discounts = [0, 10, 15, 20, 25, 30, 35, 40, 45]
    best_d = None
    for d in discounts:
        p = wine['our_reg'] * (1 - d/100.0)
        # Ищем минимальную скидку, которая проходит по марже и бьет цель
        if p >= min_margin and p <= target:
            best_d = (d, p)
            break
    ...
    if best_d is not None:
         if best_d[0] == 0:
             ...
             return f"⚔️ **Борьба скидками:** Рекомендуемая скидка **{best_d[0]}%** (Ваша цена: {best_d[1]:.0f}₽). Целевая: {target:.0f}₽."
             
    # Если фиксированные скидки не подходят (слишком дорогие или режут маржу) -> Акции
    promo = []
    stock = wine.get('stock', '3+')
    if stock in ['2', '3+']:
         p_50 = wine['our_reg'] * 0.75
         if p_50 >= min_margin: promo.append("«-50% на вторую» (экв. 25%)")
    if stock == '3+':
         p_11 = wine['our_reg'] * 0.666
         if p_11 >= min_margin: promo.append("«1+1=3» (экв. 33%)")
              
    if promo:
         return "🎁 **Пакетные акции:** Обычные скидки не дотягиваются до конкурента. Доступные акции:\n" + "\n".join([f"- {x}" for x in promo])
         
    return f"⚖️ Обычные скидки не достигают целевой цены {target:.0f}₽ без нарушения маржинальности. Отредактируйте верхнюю цену или продавайте без скидок."
